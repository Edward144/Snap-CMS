                ...
                while($item = $items->fetch_assoc()) :             
                    ...
                    $output .=
                        '<li class="item' . $item['id'] . ' ' . ($checkChildren > 0 ? 'hasChildren' : '') . ' ' . ($this->hasImages == true ? 'hasImages' : '') . '">
                            <a href="' . (strpos($item['url'], 'http') === false && $item['url'] != '#' ? ROOT_DIR . $item['url'] : $item['url']) . '" id="' . ($itemUrl == ROOT_DIR . $item['url'] ? 'active' : '') . '">' . $item['name'] . ($checkChildren > 0 ? '<span id="arrow"> ▼</span>' : '') . '</a>';
